...
- **TURN server support** — the single biggest reliability upgrade for spotty networks. STUN alone (what we had before) fails on strict wifi (corporate, college, some carriers); TURN is a relay fallback for exactly those cases. **You need to add free credentials for this to activate**: sign up free at [metered.ca/tools/openrelay](https://www.metered.ca/tools/openrelay/), copy your TURN username + credential from the dashboard, then:
  - **Web app**: before `app.js` loads, set `window.TOGETHER_TURN_USERNAME` and `window.TOGETHER_TURN_CREDENTIAL` (add a small `<script>` tag in `index.html`, or wherever you're comfortable configuring it).
  - **Android app**: paste them into the `turnUsername` / `turnCredential` fields near the top of `RTCClient.kt`.
  - Calls still work fine via STUN without this — TURN just makes them reliable on trickier networks too.
...
This app already uses the same core call technology (WebRTC) both of those use. For a 1-on-1 call, the above changes get you genuinely close to their quality. Where they still pull ahead — global relay server networks, years of audio-processing tuning, large-scale infrastructure — isn't realistically something to chase for a two-person app; the improvements above are the ones that actually move the needle for your use case.
